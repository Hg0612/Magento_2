...
namespace Lof\MpFaq\Controller\Marketplace\FaqSetting;

class EnableProduct extends \Magento\Framework\App\Action\Action
{
    /**
     * Core registry
     *
    ...
    public function execute()
    {
        ...
        if ($id) {
            $productId = $id;
            $statusModel = $this->_objectManager->create('Lof\MpFaq\Model\EnableProduct')->load($productId);
            $productModel = $this->_objectManager->create('Lof\MarketPlace\Model\SellerProduct')->load($productId);
            $sellerId = $this->_sellerHelper->getSellerId();

            if ($productModel->getSellerId() != $sellerId) {
                $this->messageManager->addError(__('This product no longer exists.'));
                return $this->_redirect('catalog/faqsetting/index');
            }

            if(!$statusModel->getProductId()){
                $data = [
