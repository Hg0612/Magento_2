...
class Author extends \Magento\Framework\View\Element\AbstractBlock implements DataProviderInterface
{
    /**
    ...
    public function getRssData()
    {
        ...
        try {
            ...
            $author = $this->_userFactory->create()->getCollection()->addFieldToFilter("username", $alias)->getFirstItem(); 
        ...
        $authorrss = $this->_blogHelper->getConfig("general_settings/authorrss");
        if(empty($author->getData()) || !$authorrss){
            return [
                'title' => 'Posts Not Found',
                'description' => 'Posts Not Found',
                'link' => $this->getUrl(''),
        ...
        $newUrl = $this->_blogHelper->getAuthorUrl($author);
        $title = $author->getFirstname() . ' ' . $author->getLastName();
        $data = ['title' => $title, 'description' => $author->getEmail(), 'link' => $newUrl, 'charset' => 'UTF-8'];
        ...
        ->addFieldToFilter('user_id', $author->getUserId())
