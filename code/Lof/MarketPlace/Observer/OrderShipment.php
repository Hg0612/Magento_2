...
class OrderShipment implements ObserverInterface {
    ...
    public function execute(\Magento\Framework\Event\Observer $observer) {
        ...
        foreach($sellerShipment as $sellerId => $items){
            ...
            $sellerOrder->setStatus(Order::STATE_COMPLETE);
            $sellerOrder->save();
            $description = __('Amount from order').' #'.$sellerOrder->getOrderId().','.__('invoice').' #'.$sellerInvoice->getInvoiceId();
            
              
            
            $this->updateSellerAmount ( $sellerId, $sellerOrder->getSellerAmount(),$description);
