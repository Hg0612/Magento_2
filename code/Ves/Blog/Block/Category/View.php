...
class View extends \Magento\Framework\View\Element\Template
{
    ...
    {   
        $category = $this->getCategory();
        $page_title = $category->getPageTitle();
        $page_title = $page_title?$page_title:$category->getName();
    ...
    protected function _beforeToHtml()
    {
        ...
        if($orderby == 1){
            $postCollection->getSelect()->order('main_table.creation_time DESC');
        }else if($orderby == 2){
            $postCollection->getSelect()->order('main_table.creation_time ASC');
        }else if($orderby == 3){
