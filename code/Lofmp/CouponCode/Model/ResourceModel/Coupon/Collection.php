...
namespace Lofmp\Couponcode\Model\ResourceModel\Coupon;
...
class Collection extends AbstractCollection
{
    ...
    {
        $this->_init('Lofmp\CouponCode\Model\Coupon', 'Lofmp\CouponCode\Model\ResourceModel\Coupon');
    }
    	...
    	if(count($items)){
          ...
          $couponId = $item->getData('coupon_id');
    			$select = $connection->select()
                                    ...
                                    ->join(
                                        array('lofrule' => $this->getTable('lofmp_couponcode_rule')), 
    ...
    public function getByCouponCode($coupon_code){
      $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $SellerPost = $objectManager->get('Lofmp\CouponCode\Model\Coupon')->load (
            $coupon_code, 'code');
    ...
    public function getCouponCodeByConditions($param){
      ...
      $select = 'SELECT *  FROM ' . $this->getTable('salesrule_coupon') . ' INNER JOIN '. $this->getTable('lofmp_couponcode_coupon').' ON salesrule_coupon.coupon_id = lofmp_couponcode_coupon.couponcode_id';
      $where = '';
      $numItems = count($param);
        $i = 0;
        foreach ($param as $key => $value) {
          if(++$i === $numItems){
            $where .=' lofmp_couponcode_coupon.'. $key . ' = ' . '"' . $value . '"';
          } else{
            $where .=' lofmp_couponcode_coupon.'. $key . ' = ' . '"' . $value . '" and ';
          }
        }
      if($where != ''){
        $where = ' WHERE '. $where;
