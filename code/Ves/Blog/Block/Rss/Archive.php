...
class Archive extends \Magento\Framework\View\Element\AbstractBlock implements DataProviderInterface
{
    /**
    ...
    public function getRssData()
    {
        ...
        $latestpagerss = $this->_blogHelper->getConfig("general_settings/archiverss");
        if(!$latestpagerss){
            return [
        ...
        $time = $this->getRequest()->getParam('key');
        $time = explode('-', $time);
        $month = $time[1];
        $year = $time[0];

        $newUrl = $this->_blogHelper->getLatestPageUrl();
        $title = $this->_blogHelper->getConfig('blog_latest_page/page_title');
        ...
        ->where('MONTH(creation_time) = ?', (int)$month)
