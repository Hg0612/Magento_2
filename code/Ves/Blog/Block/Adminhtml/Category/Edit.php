 ...
 */
namespace Ves\Blog\Block\Adminhtml\Category;
...
class Edit extends \Magento\Backend\Block\Widget\Form\Container
{
    ...
    protected function _construct()
    {
        ...
        if ($this->_isAllowedAction('Ves_Blog::category_save')) {
        ...
        if ($this->_isAllowedAction('Ves_Blog::category_delete')) {
    ...
    public function getHeaderText()
    {
        if ($this->_coreRegistry->registry('blog_category')->getId()) {
            return __("Edit Category '%1'", $this->escapeHtml($this->_coreRegistry->registry('blog_category')->getTitle()));
        } else {
