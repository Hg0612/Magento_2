                ...
                if ($sellerApproval) {
                    ...
                    try {
                        ...
                        $this->_eventManager->dispatch(
                            ...
                            ['controller' => $this,'seller' => $customerObject]
                        );
                    ...
                    $sellerModel = $objectManager->get('Lof\MarketPlace\Model\Seller');
                    try {
                        $sellerModel->setName($customerName)->setEmail($customerEmail)->setStatus(1)->setGroupId($group)->setCustomerId($customerId)->setUrlKey($url)->setPageLayout($layout)->save();
                         $this->_eventManager->dispatch(
                            'controller_action_seller_save_entity_after',
                            ['controller' => $this,'seller' => $customerObject]
                        );
