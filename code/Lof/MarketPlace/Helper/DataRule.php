...
Class DataRule extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    */
    ...
    { 
        ...
        $rules = $this->getRules()->addFieldToFilter('group_id',array('finset'=>$seller->getData('group_id')))->addFieldToFilter('is_active',1)->setOrder('priority', 'ASC');
        ...
        foreach($commissionRules as $commissionRule) {
            foreach ($rules as $key => $_rules) {
                if((in_array($storeId, $_rules->getData('store_id')) || in_array(0, $_rules->getData('store_id'))) && count($_rules) >0 ){
