 ...
 */
namespace Lof\Slider\Model\ResourceModel;
...
class Slider extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('lof_marketplace_slider', 'slider_id');
    }
    ...
    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('lof_marketplace_slider'), $condition);
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        if($stores = $object->getStores()){
            $table = $this->getTable('lof_marketplace_slider_store');
    ...
    protected function _afterLoad(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if ($id = $object->getId()) {
            ...
            ->from($this->getTable('lof_marketplace_slider'))
            ->where(
                'slider_id = '.(int)$id
                );
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['cbs' => $this->getTable('lof_marketplace_slider_store')],
                $this->getMainTable() . '.slider_id = cbs.slider_id',
                ['store_id']
                )->where(
                'is_active = ?',
                1
    ...
    public function lookupStoreIds($id)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('lof_marketplace_slider_store'),
            'store_id'
            )->where(
            'slider_id = :slider_id'
            );
