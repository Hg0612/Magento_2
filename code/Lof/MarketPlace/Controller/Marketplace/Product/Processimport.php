                    ...
                    foreach($data as $key => $_data) {
                        if($key) {
                            ...
                            if($productId) {
                                ...
                                $product->setData('seller_id',$this->helper->getSellerId())->save(); 
                                if(count($sellerproduct->getData()) >0) {
                                  
                                    $connection->query('UPDATE ' . $table_name . ' SET seller_id = '.$this->helper->getSellerId().',status = 0 WHERE  product_id = '.(int)$productId);
                                    
                                } else {
                                $connection->query('INSERT INTO ' . $table_name . ' (seller_id,product_id,status) VALUES ( ' . $this->helper->getSellerId() . ', ' . (int)$productId . ', 0)');  
                                } 
                            } 
                        }
                    }
