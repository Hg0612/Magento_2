...
class Edit extends \Magento\Backend\App\Action
{
    ...
    {
        return $this->_authorization->isAllowed('Lof_MarketPlace::payment_edit');
    }
    ...
    public function execute()
    {
        ...
        $model = $this->_objectManager->create('Lof\MarketPlace\Model\Payment');
        ...
        $this->_coreRegistry->register('lof_marketplace_payment', $model);

        // 5. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Seller') : __('New Payment'),
            $id ? __('Edit Seller') : __('New Payment')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Sellers'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getname() : __('New Payment'));
